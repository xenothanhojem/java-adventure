...
/**
 * Calls the Anthropic API to review Angelo's code submission.
 * Returns a structured review object, or throws if the call/parsing fails.
 */
async function reviewCode({ problem, criteria, code }) {
  const systemPrompt = `You are a friendly Java tutor reviewing code from Angelo, a Grade 10 student in South Africa learning Java. He uses NetBeans IDE and follows the textbook "Exploring IT: Java Programming Grade 10".
...
Keep strengths and improvements arrays to 1-3 short items each. If there's nothing to put in one, use an empty array.`;
  ...
  // Strip code fences just in case Claude added them
  const cleaned = text
    .replace(/^```json\s*/i, '')
    .replace(/^```\s*/i, '')
    .replace(/```\s*$/i, '')
